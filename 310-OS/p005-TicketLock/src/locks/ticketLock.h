...
class TicketLock {
private:
  /* uint turn - the current turn that the ticket dispenser is on
   * uint nowServing - the thread making progress
   * uint totalTurns - this is used to create the cyclic nature of the lock
   * SpinLock *spinlock - This is used to make sure that evil_increment() actually increment.
   */
  unsigned int turn;
  unsigned int nowServing;
